...
| 'real' | '/' | '+' | '<-' | '*' | '-' | 'logico' | '<' | 'e' | '=' | 'nao' | 'ou' | '<=' | 'se' | '>=' | 'entao' | 'senao' 
| 'fim_se' | 'caso' | 'seja' | '..' | 'fim_caso' | 'para' | 'ate' | 'faca' | 'fim_para' | 'enquanto' | '<>' | 'fim_enquanto' 
| '>' | '%' | '^' | '&' | 'registro' | 'fim_registro' | '.' | 'tipo' | 'procedimento' | 'var' | 'fim_procedimento' | 'funcao' 
